...
//                NSString *da = [[NSString alloc] initWithFormat:@"%lu",(unsigned long)i];
                NSString *index = [[NSString alloc] initWithFormat:@"%lu",(unsigned long)i];
                NSMutableArray *da = [[NSMutableArray alloc] initWithObjects:index, perpherals[i].name, nil];
                [myArray addObject:da];
            }
            self.deviceArray = perpherals;
            PDRPluginResult * result = [PDRPluginResult resultWithStatus:PDRCommandStatusOK messageAsArray:myArray];
...
- (void)MyPluginTest:(PGMethod*)commands{
    if(commands){
        NSString* cbId = [commands.arguments objectAtIndex:0];
        NSString* index = [commands.arguments objectAtIndex:1];
        NSLog(@"%@",index);
        CBPeripheral *pre = [self.deviceArray objectAtIndex:0];
        PDRPluginResult * result = [PDRPluginResult resultWithStatus:PDRCommandStatusOK messageAsArray:pre.name];
        result.keepCallback = true;
